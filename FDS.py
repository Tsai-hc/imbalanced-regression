# -*- coding: utf-8 -*-
"""
Created on Fri Jun  9 16:20:05 2023
...
class FDS(nn.Module):
    ...
    def __init__(self, feature_dim, bucket_num=50, bucket_start=4, start_update=0, start_smooth=1,
                 kernel='gaussian', ks=2, sigma=10, momentum=0.1):
...
class My_Model(nn.Module):
    def __init__(self, input_dim):
        ...
        self.FDS = FDS(
                ...
                start_update=0, start_smooth=1, kernel='gaussian', ks=9, sigma=1,
                momentum = 0.9
            )
...
def trainer(train_loader, valid_loader, model, config, device):
        ...
        #print(f"FDS train.")
            ...
            with torch.no_grad():
                for x, y in train_pbar:
                    ...
                    encodingss.extend(feature.data.squeeze().cpu().numpy())
                    labelss.extend(y.data.squeeze().cpu().numpy())

            encodings, labels = torch.from_numpy(np.vstack(encodingss)).cuda(), torch.from_numpy(np.hstack(labelss)).cuda()
            model.FDS.update_last_epoch_stats(epoch)
            q ,qq = model.FDS.update_running_stats(encodings, labels, epoch)
            print(encodings)
    ...
    return encodingss ,labelss,q,qq
...
a ,aa ,aq, aqq= trainer(train_loader, valid_loader, model, config, device)
